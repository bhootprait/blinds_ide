...
def custom_quit(): #exit the program
...
def save(): #to save the existing file
    global current_open_file
    if current_open_file=="no_file":
        save_as_file()
    else:
...
def shortcut_keys(event): #Speak all the shortcut keys
...
def specialchar(data): #convert special characters to their name
...
def errshort(data): #shorten the error
...
def allcode(event): #speaks all code
...
def output(ar_out): #update output window
...
def cursor_move(l): #move text cursor to error line
...
def err(ar_error): #update error window
...
def user_input_to_program(user_input,prog): #takes input , replace program
    k=user_input.split(" ")
    k.pop(0)
    k.pop(0)
    print("k",k)
...
def p(): #compile
    ...
    if os.path.exists('error.txt'):
        os.remove('error.txt')
    text_file = open("a.py", "r")
    inputValue=text_file.read()
    text_file.close()
    textpad.delete('1.0',END)
    textpad.insert(INSERT,inputValue)
    output(ar_out)
    err(ar_error)
    
def motion(event): #speak where you are
    say("this is your input window ")
    return
def output_window(event): #speak where you are
    say("this is your output window ")
    return
def error_window(event): #speak where you are
  say("this is your error window ")
  return 
def writing_window(event): #speak where you are
...
def fg(): #update flag
    global flg
    flg=0
class RepeatedTimer: #calls a function in a particular time interval
...
timer = RepeatedTimer(3, fg) #calls fg every 3 seconds
...
mouse = Controller() #helps in moving mouse pointer
...
def motion_comp(event): #speak compile
  say("this is your compile option you can click to compile ")
  return

def com(event): #compile
    global flg
    if flg==0:
        p()
        flg=1
...
def valuebhej(evt): #update the value with suggested value
    global completions
...
def CurSelet(evt): #moves the selected value for updation
    value=str((text_auto.get(text_auto.curselection())))
    say(value)
    print (value)
    text_auto.bind('<Double-1>', valuebhej)
    
def auto_name(event): #speak name
    say("This is your auto complete window")
    
def run(event): #compile
    p()
    
def cal_pos(): #used in indentation returns current line
    line=textpad.index(INSERT)
    val=float(line)
    y=line
    cnt=0
...
def iden(whole): #used in indentation inserts tab spaces
    print("this is")
    global flag_colon
...
def check_iden(event): #indentation
    imple()
    
       
def imple(): #insentation
    global flag_colon
    l=cal_pos()
    whole=l[0]
    s=l[1]
...
def remove_iden(event): #remove indentation
    global flag_colon
    line=textpad.index(INSERT)
    print(line)
    print (str(float(int(float(line))+1)))
...
def speak_output(event): #speaks current output
    value=text.get("1.0",END)
    value=value[:-1]
    value=specialchar(value)
    say(value)

def speak_input(event): #speaks current input
    value=text3.get("1.0",END)
    value=value[:-1]
    value=specialchar(value)
    say(value)
        
def speak_error(event): #speaks current error
